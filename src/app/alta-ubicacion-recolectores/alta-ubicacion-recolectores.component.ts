...
@Component({
  selector: 'app-alta-ubicacion-recolectores',
  templateUrl: './alta-ubicacion-recolectores.component.html',
  styleUrls: ['./alta-ubicacion-recolectores.component.css'],
  providers: [ServicioAlertasRecolectoresUbi, ServicioInfoRecolectores, Modales]
})
export class AltaUbicacionRecolectoresComponent {
  ...
  adelPagina(): void{
    ...
    for (let i = 0; i < this.tabla.length; i++) {
      if (i != 0) {
        estados.push(this.tabla[i].estado);
        let arr:string[] = []
        municipio.push(arr);
        for (let j = 0; j < this.tabla[i].municipios.length; j++) {
          municipio[i-1].push(this.tabla[i].municipios[j]);      
        }
      }
    ...
    this.servicioInfo.pushUbi(estados,municipio);
    this.modal.disp();
